...
export async function runHeartbeatOnce(opts: {
  ...
  // Append knowledge base context so the agent knows about recent KB activity.
  try {
    const kbPath = (cfg as Record<string, unknown>).knowledgeBase
      ? ((cfg as Record<string, unknown>).knowledgeBase as { vaultPath?: string })?.vaultPath
      : undefined;
    const kbContext = await resolveKBContextForHeartbeat(kbPath);
    if (kbContext) {
