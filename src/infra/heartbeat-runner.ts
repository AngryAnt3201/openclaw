...
function resolveHeartbeatReasonFlags(reason?: string): HeartbeatReasonFlags {
  ...
  return {
    ...
    isTaskEventReason: reasonKind === "task",
...
async function resolveHeartbeatPreflight(params: {
  ...
  const shouldInspectPendingEvents =
    reasonFlags.isExecEventReason || reasonFlags.isCronEventReason || reasonFlags.isTaskEventReason || hasTaggedCronEvents;
  const shouldBypassFileGates =
    reasonFlags.isExecEventReason ||
    reasonFlags.isCronEventReason ||
    reasonFlags.isTaskEventReason ||
...
export async function runHeartbeatOnce(opts: {
  ...
  const { isCronEventReason, isTaskEventReason, pendingEventEntries } = preflight;
  ...
  const hasTaskEvents = isTaskEventReason && pendingEvents.length > 0;
