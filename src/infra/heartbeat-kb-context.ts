...
// ---------------------------------------------------------------------------
// Reads the knowledge base and formats a summary of recent notes so the
// heartbeat agent has visibility into knowledge base activity.
// ---------------------------------------------------------------------------
 ...
 * Returns `null` if no recent notes or KB path is not configured.
 */
export async function resolveKBContextForHeartbeat(
  ...
  try {
    const allNotes = await listNotes(kbPath);
    const now = Date.now();
    ...
    return (
      ...
      lines.join("\n") +
      "\n\nYou can create/update knowledge base notes to persist important information."
    );
  } catch {
    return null;
  }
