...
import { MIRANDA_AGENT_ID } from "../agents/builtin/index.js";
...
describe("gateway server chat", () => {
  test("smoke: caps history payload and preserves routing metadata", async () => {
    ...
    try {
      ...
      >;
      expect(stored[`agent:${MIRANDA_AGENT_ID}:main`]?.lastChannel).toBe("whatsapp");
      expect(stored[`agent:${MIRANDA_AGENT_ID}:main`]?.lastTo).toBe("+1555");
    } finally {
