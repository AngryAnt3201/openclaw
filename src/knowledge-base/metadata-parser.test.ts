...
describe("word count (via parseNoteMetadata)", () => {
  ...
  it("returns 1 for a single word", () => {
    const result = parseNoteMetadata("hello");
    expect(result.wordCount).toBe(1);
  });

  it("counts words in a paragraph correctly", () => {
    const result = parseNoteMetadata("The quick brown fox jumps over the lazy dog.");
    expect(result.wordCount).toBe(9);
  });
});

// ---------------------------------------------------------------------------
// 7. Full parseNoteMetadata
// ---------------------------------------------------------------------------
...
describe("parseNoteMetadata", () => {
  it("extracts all metadata from a combined document", () => {
    ...
    const meta = parseNoteMetadata(content);
